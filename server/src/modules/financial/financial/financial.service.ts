...
export class FinancialService {
  ...
  async getPendingTasks(
    ...
    // Lấy danh sách công việc từ nhiều nguồn
    const today = new Date();
    
    // Lấy dữ liệu từ các nguồn khác nhau
    const maintenanceTasks = await this.getPendingMaintenanceTasks(userId, today);
    const billTasks = await this.getPendingBillTasks(userId, today);
    const contractTasks = await this.getContractsEndingSoon(userId, today);
    
    // Chuyển đổi dữ liệu từ các nguồn
    const maintenanceTasksList = this.mapMaintenanceTasksToPendingTasks(maintenanceTasks, today);
    const billTasksList = this.mapBillsToPendingTasks(billTasks);
    const contractTasksList = this.mapContractsToPendingTasks(contractTasks);
    
    // Kết hợp và sắp xếp tất cả các tasks
    const allTasks = [...maintenanceTasksList, ...billTasksList, ...contractTasksList];
