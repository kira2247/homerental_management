...
import { Property, PropertyStatus, PropertyType } from "@/lib/api/types";
...
export function PropertyOverviewTab({ property }: PropertyOverviewTabProps) {
  ...
  const getStatusBadge = (status: PropertyStatus) => {
    const statusConfig = {
      [PropertyStatus.AVAILABLE]: { className: "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300", label: status },
      [PropertyStatus.OCCUPIED]: { className: "bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300", label: status },
      [PropertyStatus.UNDER_MAINTENANCE]: { className: "bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-300", label: status },
      [PropertyStatus.INACTIVE]: { className: "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300", label: status }
    };
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {t('properties.propertyDetail.utilities.waterValue', { rate: formatNumber(property.defaultWaterRate) })}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                  ...
                  <span className="font-medium">
                    {t('properties.propertyDetail.utilities.electricityValue', { rate: formatNumber(property.defaultElectricityRate) })}
                  </span>
                </div>
              </div>
            </div>
