...
export default function DashboardContent() {
  ...
  const [propertyData, setPropertyData] = useState<PropertyDistributionDto | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isApiSuccess(tasksResponse as ApiResponse<any>) && tasksResponse.data && tasksResponse.data.tasks) {
          ...
          const mappedTasks: Task[] = tasksResponse.data.tasks.map((task: PendingTaskDto) => ({
            id: task.id,
            title: task.title,
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        <OverviewCard 
          ...
          isLoading={isLoading} 
          period={period === 'week' ? 'month' : period}
          className="h-full"
        />
      </div>
