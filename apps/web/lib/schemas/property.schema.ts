...
export const propertySchema = z.object({
  ...
  thumbnail: z.union([
    z.literal(""),
    z.string().url("URL hình ảnh thu nhỏ không hợp lệ.")
  ]).optional().nullable()
    .describe("URL của hình ảnh đại diện chính cho bất động sản (Không bắt buộc)."), // Optional

  // --- Images ---
  images: z.array(z.string().url("URL hình ảnh không hợp lệ"))
    .optional()
    .describe("Danh sách các URL hình ảnh của bất động sản (Tối đa 10 URL)."),
  ...
  // ownerId: z.string().uuid(), // Sẽ được xử lý riêng, không cần trong form data trực tiếp từ user
  // userId: z.string().uuid(), // Sẽ được xử lý riêng
});
