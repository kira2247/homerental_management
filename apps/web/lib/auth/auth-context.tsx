  ...
  useEffect(() => {
    ...
    const checkAuthStatus = async () => {
      try {
        ...
        console.log('🔍 [AuthContext] Bắt đầu kiểm tra trạng thái xác thực...');
        ...
        console.log('📡 [AuthContext] Gọi API /api/auth/me để xác thực...');
        try {
          ...
          if (!(isAuthPage && response.status === 401)) {
            console.log('📡 [AuthContext] Phản hồi từ /api/auth/me - Status:', response.status, response.statusText);
          }
          
          if (response.ok) {
            const data = await response.json();
            console.log('✅ [AuthContext] Dữ liệu phản hồi từ /api/auth/me:', data);
            
            if (data.success && data.user) {
              console.log('👤 [AuthContext] Xác thực thành công, cập nhật state user:', data.user);
              if (isMounted) setUser(data.user);
            } else {
              // API trả về 200 nhưng không thành công hoặc không có user
              console.log('❌ [AuthContext] Phản hồi API OK nhưng không hợp lệ, đặt user = null');
            ...
            if (!(isAuthPage && response.status === 401)) {
              console.log(`❌ [AuthContext] Phản hồi API không thành công (${response.status}), đặt user = null`);
            }
            ...
            if (response.status === 401 && !isAuthPage) {
              console.log('🔑 [AuthContext] Lỗi 401 - Có thể token hết hạn hoặc không hợp lệ. Thử làm mới token...');
              // Tự động thử làm mới token - chỉ khi không phải ở trang xác thực
              const refreshed = await refreshToken();
              if (refreshed) {
                 console.log('🔄 [AuthContext] Làm mới token thành công. Sẽ thử lại checkAuthStatus...');
                 ...
                 console.log('❌ [AuthContext] Làm mới token thất bại.');
        ...
        if (isMounted) {
          ...
          if (!isAuthPage) {
            console.log('🏁 [AuthContext] Kết thúc kiểm tra xác thực, đặt isLoading = false.');
          }
          setIsLoading(false);
        }
      }
