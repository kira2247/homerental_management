...
import { usePropertyApi, CreatePropertyRequest, UpdatePropertyRequest, Property, ApiResponse, isApiSuccess } from '@/lib/api';
import { useAuthStatus } from '@/lib/hooks/auth';
import { propertyKeys, toQueryKey } from './property-keys';
import { optimisticListUpdate } from './optimistic-update';
...
export function useCreateProperty(options: UsePropertyMutationOptions = {}) {
  ...
  return useMutation<Property, Error, CreatePropertyRequest>({
    mutationFn: async (data: CreatePropertyRequest): Promise<Property> => {
      ...
      try {
        ...
        if (isApiSuccess(response) && response.data) {
          ...
          // Xử lý lỗi từ API
          const errorMessage = response.error?.message || 'Không thể tạo bất động sản';
          console.error('[useCreateProperty] API trả về lỗi:', errorMessage);
    ...
    onError: (error) => {
      console.error('[useCreateProperty] onError:', error);
    }
  });
}
...
export function useUpdateProperty(options: UsePropertyMutationOptions = {}) {
  ...
  return useMutation<Property, Error, { id: string; data: UpdatePropertyRequest }>({
    mutationFn: async ({ id, data }: { id: string; data: UpdatePropertyRequest }): Promise<Property> => {
      ...
      try {
        ...
        if (isApiSuccess(response) && response.data) {
          ...
          // Xử lý lỗi từ API
          const errorMessage = response.error?.message || 'Không thể cập nhật bất động sản';
          console.error('[useUpdateProperty] API trả về lỗi:', errorMessage);
          throw new Error(errorMessage);
        }
      } catch (error) {
    ...
    onError: (error, { id }, context: PropertyMutationContext | undefined) => {
      console.error('[useUpdateProperty] onError:', error);
      
      // Khôi phục dữ liệu gốc nếu có
      if (context && context.previousProperty) {
        optimisticUpdate.rollbackPropertyData(id, context.previousProperty);
      }
    },
    onSettled: (_, __, { id }) => {
      if (autoInvalidateQueries) {
...
export function useDeleteProperty(options: UsePropertyMutationOptions = {}) {
  ...
  return useMutation<void, Error, string>({
    ...
    onError: (error) => {
      console.error('[useDeleteProperty] onError:', error);
    }
  });
}
...
export function useUpdatePropertyStatus(options: UsePropertyMutationOptions = {}) {
  ...
  return useMutation<Property, Error, { id: string; status: string }>({
    ...
    onError: (error, { id }, context: PropertyMutationContext | undefined) => {
      console.error('[useUpdatePropertyStatus] onError:', error);
...
export function useUploadPropertyImages(options: UsePropertyMutationOptions = {}) {
  ...
  return useMutation<string[], Error, { id: string; images: File[] }>({
    mutationFn: async ({ id, images }): Promise<string[]> => {
      ...
      try {
        ...
        if (isApiSuccess(response) && response.data) {
          ...
          const errorMessage = response.error?.message || 'Không thể tải lên hình ảnh bất động sản';
          console.error('[useUploadPropertyImages] API trả về lỗi:', errorMessage);
          throw new Error(errorMessage);
        }
      } catch (error) {
    ...
    onError: (error) => {
      console.error('[useUploadPropertyImages] onError:', error);
    }
  });
} 