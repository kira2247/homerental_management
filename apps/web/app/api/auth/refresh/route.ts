...
export async function POST(request: NextRequest) { 
  try {
    ...
    if (!refreshToken) {
      if (!isSilentCheck) {
        console.log('[API] auth/refresh: Không tìm thấy refresh token trong cookie');
      }
      return NextResponse.json(
    ...
    if (!isSilentCheck) {
      console.log(`[API] auth/refresh: Gọi backend API ${backendUrl}/api/auth/refresh-token`);
    }
    ...
    try {
      ...
      if (!isSilentCheck) {
        console.log(`[API] auth/refresh: Phản hồi từ backend - Status: ${response.status}`);
      }
      ...
      if (!isSilentCheck) {
        console.log(`[API] auth/refresh: Nhận được token mới từ backend API`);
      }
      ...
      if (!newToken) {
        if (!isSilentCheck) {
          console.error('New token not found in API response');
        }
        return NextResponse.json(
      ...
      if (newRefreshToken) {
        if (!isSilentCheck) {
          console.log('[API] auth/refresh: Cập nhật refresh token mới');
        }
        cookies().set({
          name: 'refresh_token',
    ...
    const isSilentCheck = request.headers.get('X-Silent-Auth-Check') === '1';
    if (!isSilentCheck) {
      console.error('Refresh token general error:', error);
    }
